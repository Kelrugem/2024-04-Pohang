...
\setbeamertemplate{footline}
{
  ...
  \begin{beamercolorbox}[wd=.91\paperwidth,ht=2.25ex,dp=1ex,center]{title in head/foot}%
    \usebeamerfont{title in head/foot}
		Graphic by Camille Laurent-Gengoux, Ruben Louis, and Leonid Ryvkin.
  \end{beamercolorbox}%
  %\begin{beamercolorbox}[wd=.175\paperwidth,ht=2.25ex,dp=1ex,right]{date in head/foot}%
    %\usebeamerfont{date in head/foot}\insertshortdate{}\hspace*{2em}
    %%\insertframenumber{} 
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=1\textwidth]{magnetique.png}
	\caption{The magnetic partition}
...
\begin{itemize}
	\item All leaves are of the same dimension.
	\item \textbf{But:} It lacks regularity!
\end{itemize}
\end{remark}
\end{minipage}
\end{frame}
...
\begin{frame}
\begin{definition}[Smooth partitionifold]
...
\begin{example}[Vector fields not necessarily finitely generated]
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=.9\textwidth]{snake.png}
	\label{fig:snake}
\end{figure}
%\end{center}
%\end{minipage}
...
\subsection{More on foliation connections}
...
%\end{tikzcd}
\begin{remark}
These connections are called \textbf{(multiplicative) Yang-Mills connections}:
\begin{itemize}
...
\begin{tikzcd}[ampersand replacement=\&]
\&\{\text{YM connection on } \mathcal{T}\} \arrow{dd} \arrow{dl}\arrow{dr}\&
\\
\arrow{dr} \text{Foliation } \mathcal{F} \arrow{ur}\&\& \arrow{ul} \arrow{dl} ?
\\
\&\{\text{mult.\ YM connection on } \mathup{Sym}(\tau)\} \arrow{uu} \arrow{ul}\arrow{ur}\&
\end{tikzcd}
\end{frame}
}
...
\begin{frame}
\begin{corollary}[{[C.\ L.-G., S.-R.\ F.]}]
$\mathup{Sym}(\tau)$ is associated to a principal $\mathup{Sym}(\tau_l)$-bundle so that the multiplicative Yang-Mills connection on $\mathup{Sym}(\tau)$ is an associated connection (adjoint connection).
\end{corollary}
\pause
\begin{proof}
$\mathup{Sym}\mleft(\tau_l, \tau_{l'}\mright)$ gives rise to a transitive groupoid with isotropy group bundle $\mathup{Sym}(\tau)$ which has no centre.
\end{proof}
\end{frame}
...
\begin{enumerate}
	\item Only need $G \coloneqq \mathrm{Inn}(\tau_l)$
	\pause
	\item $P$ a principal $G$-bundle, equipped with an ordinary connection
	%\pause
	%\item 
	\pause
	\item $\mathup{Inner}(\tau) \coloneqq (P \times G) \Big/ G$, the \textbf{inner group bundle}
	\pause
...
\begin{itemize}
	...
	\item $\mathup{Inner}(\tau)$ acts on $\mathcal{T}$ (canonically from the left).
\end{itemize}
\end{remark}
...
The associated connection on $\mathup{Inner}(\tau)$ is a multiplicative Yang-Mills connection and the one on $\mathcal{T}$ is a corresponding Yang-Mills connection.
\end{proposition}
...
[p, g \cdot v]
\ees
for all $[p, g] \in \mathup{Inner}(\tau)$ and $[p, v] \in \mathcal{T}$, and
\bes
...
\begin{proof}
\begin{itemize}
	\item The adjoint bundle of $P$, $\mathup{Ad}(P) \coloneqq (P \times \tau_l)/G$, is the Lie algebra bundle of $\mathup{Inner}(\tau)$
...
\\
\&\{\text{mult.\ YM onnection on } \mathup{Sym}(\tau)\} \arrow{uu} \arrow{ul}\arrow{ur}\&
\end{tikzcd}
...
\renewcommand\insertreferences{{\tiny Corollary by Camille Laurent-Gengoux and Leonid Ryvkin, The neighborhood of a singular leaf, \newline \textit{Journal de l'\'Ecole Polytechnique}, Math.\ 8, 1037 - 1064, 2021.}}
